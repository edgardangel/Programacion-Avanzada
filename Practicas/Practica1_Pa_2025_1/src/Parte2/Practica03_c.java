...
public class Practica03_c extends JFrame implements ActionListener {
    ...
    private ListaCategorias_c listaCategorias;
    private Archivotxt archivito;
    private final String nombreArchivo = "categorias.txt"; 
    ...
    public Practica03_c() {
        ...
        listaCategorias = new ListaCategorias_c();

        archivito = new Archivotxt(nombreArchivo);
    ...
    private void cargarDesdeArchivo() {
        ArrayList<String> elementos = archivito.cargar();
        ArrayList<String[]> listaTemporal = new ArrayList<>();

        for (int i = 0; i < elementos.size(); i += 2) {
            if (i + 1 < elementos.size()) {
                String[] par = { elementos.get(i), elementos.get(i + 1) };
                listaTemporal.add(par);
            }
        }
        listaCategorias.cargarCategorias(listaTemporal);
    }

    private void guardarEnArchivo() {
        StringBuilder sb = new StringBuilder();
        Object[] cats = listaCategorias.CategoriasArreglo();
        for (Object catObj : cats) {
            Categoria cat = (Categoria) catObj;
            sb.append(cat.getIdcategoria()).append(", ")
              .append(cat.getCategoria()).append("\n");
        }
        archivito.guardar(sb.toString());
    }
